...
class DANN(nn.Module):
    def __init__(
        ...
        self.decoder1 = Knowledge_Decoder(input_size=50,output_size=100)
        ...
        self.classifer = news_classifier(input_size=domain_size,hidden_size=hidden_size,hidden_size_2=hidden_size_2,num_class=num_classes)
        #self.reverselayer = ReverseLayerF()
        
    def forward(self,text,image_path,knowledge,domain_label):
        #text,image_path,domain_label = batch
        fused_features = self.encoder1(text,image_path)
        ...
        classification_logits = self.classifer(content_feature)
        return discriminator_logits,classification_logits,reconstructed
